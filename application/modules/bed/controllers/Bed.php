...
class Bed extends MX_Controller {
    ...
    function addAllotment() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $a_timestamp = explode('-', $a_time);
            $d_timestamp = explode('-', $d_time);
            $data = array(
                'bed_id' => $bed_id,
                'patient' => $patient,
                'a_time' => $a_time,
                'd_time' => $d_time,
                'a_timestamp' => strtotime($a_timestamp[0] . ' ' . $a_timestamp[1]),
                'd_timestamp' => strtotime($d_timestamp[0] . ' ' . $d_timestamp[1]),
                'status' => $status,
